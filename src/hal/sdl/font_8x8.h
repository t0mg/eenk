 ...
 *
 * Usage in SDLDisplay:
 *   Glyphs are rendered at 2× scale (16×16 px per character) so the desktop
 *   window feels closer to the final e-ink device's apparent DPI.
 *
 * Source: Public-domain CP437 font data.
 */
...
/** Get a pointer to a character's 8-row bitmap. Returns '?' for out-of-range. */
inline const uint8_t* fontGlyph(char c)
{
    unsigned idx = (unsigned char)c;
    if (idx < 0x20 || idx > 0x7F) idx = 0x3F; // '?'
    return FONT_8x8[idx - 0x20];
}
